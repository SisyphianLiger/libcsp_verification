...
/*
	Must have interface that is valid
	If invalid interface/name/data --> invalid
	If

	iface is fount in csp_interface.h
	csp_can_interface_data_t fount in
 */

/*@
	predicate valid_interface(csp_iface_t * iface) = \valid(iface)
								&& \valid(&(iface -> name))
