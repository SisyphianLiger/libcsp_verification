...
/* Just Wrapper functions for locking */
...
/*@ predicate valid_usart_context_ptr(usart_context_t *ctx) =
         \valid(ctx) && ctx->fd >= 0;
*/
/*@
    requires \valid_read((usart_context_t *)arg) && valid_usart_context_ptr((usart_context_t *)arg);
...
static void * usart_rx_thread(void * arg) {
	...
	//@ assert \valid(ctx) && \valid((usart_context_t *)arg);
	const unsigned int CBUF_SIZE = 400;
	//@ assert \valid(ctx) && \valid((usart_context_t *)arg) && CBUF_SIZE == 400;
	uint8_t * cbuf = malloc(CBUF_SIZE);
	//@ assert \valid(ctx) && \valid((usart_context_t *)arg);
	//@ assert CBUF_SIZE == 400 && cbuf == 0;
	// Receive loop
	while (1) {
...
/* START HERE : What we know,
 *  USART == Universal Synchronous/Asynchronous Receiver/Transmitter
 *
		typedef struct {
			 csp_usart_callback_t rx_callback;
			 void * user_data; // user_data pass on as argument
			 csp_usart_fd_t fd; // File Descriptor
			 pthread_t rx_thread; // Which thread it has
	    } usart_context_t;

	csp_usart_fd_t defined as int in usart.h
	void * data...gets written so not as important I guess?
	size_t data length must be the length of data of course

*/
/*@
    requires fd >= INT_MIN && fd < INT_MAX;
    requires \valid_read((char *)data+(0..data_length-1));
