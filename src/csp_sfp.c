 ...
 */
/*@
	predicate valid_sfp_header(sfp_header_t * header) =
		header != \null &&
		0 <= header->offset && header->offset < UINT_MAX &&
		0 <= header->totalsize && header->totalsize < UINT_MAX;
 */
/*@
	requires \valid(packet);
	ensures \valid(\result) && valid_sfp_header(\result);
 */
static inline sfp_header_t * csp_sfp_header_add(csp_packet_t * packet) {
	...
	//@ assert \valid(header) && header == (sfp_header_t *)&packet->data[packet->length] && valid_sfp_header(header);
	packet->length += sizeof(*header);
	//@ assert \valid(&(packet -> length)) && ((packet -> length) == (packet -> length) + (sizeof(*header)));
	return header;
}
